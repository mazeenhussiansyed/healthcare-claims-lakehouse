...
# COMMAND ----------
# Verify available Unity Catalog catalogs.

display(spark.sql("SHOW CATALOGS"))

# COMMAND ----------
# Configure the project namespace.
...
# COMMAND ----------
# Create Databricks-managed storage for synthetic source files.
...
# COMMAND ----------
# Verify uploaded raw Parquet source files.
...
# COMMAND ----------
# Load raw source data from the Databricks volume.
...
# COMMAND ----------
# Publish source-preserving Bronze Delta tables.
...
patients_raw.write.format("delta").mode("overwrite").saveAsTable(
    BRONZE_PATIENTS
)
providers_raw.write.format("delta").mode("overwrite").saveAsTable(
    BRONZE_PROVIDERS
)
claims_raw.write.format("delta").mode("overwrite").saveAsTable(
    BRONZE_CLAIMS
)
...
# COMMAND ----------
# Profile raw-data quality before transformation.
...
duplicate_claim_rows = (
    ...
    .where(F.col("count") > 1)
    .agg(
        F.sum(F.col("count") - F.lit(1)).alias(
            "duplicate_rows"
        )
    )
...
# remove duplicates, quarantine unsafe claims, and impute safe missing diagnoses.
...
quarantined_claims = (
    ...
    .withColumn(
        ...
        F.when(
            ...
            F.lit(
                "negative_paid_amount_and_invalid_discharge_date"
            ),
        )
        .when(
            F.col("paid_amount") < 0,
...
claims_safe_for_silver = deduplicated_claims.where(
    ~invalid_claim_condition
)
...
spark.table(BRONZE_PATIENTS).write.format("delta").mode(
    "overwrite"
).saveAsTable(SILVER_PATIENTS)

spark.table(BRONZE_PROVIDERS).write.format("delta").mode(
    "overwrite"
).saveAsTable(SILVER_PROVIDERS)

quarantined_claims.write.format("delta").mode(
    "overwrite"
).saveAsTable(QUARANTINE_CLAIMS)

silver_claims_df.write.format("delta").mode(
    "overwrite"
...
silver_results = [
    ...
    (
        "duplicate_claim_rows_removed",
        bronze_claims.count() - deduplicated_claims.count(),
    ),
    (
        "invalid_claim_rows_quarantined",
        quarantined_claims.count(),
    ),
    (
        "missing_diagnosis_imputed",
        missing_diagnoses_imputed,
    ),
    (
        "silver_claim_rows_published",
...
# COMMAND ----------
# Build Gold Delta tables for analytics and reporting.

GOLD_MONTHLY = (
    f"{CATALOG}.{SCHEMA}.gold_monthly_cost_utilization"
)
GOLD_PROVIDER = (
    f"{CATALOG}.{SCHEMA}.gold_provider_performance"
)
GOLD_DIAGNOSIS = (
    f"{CATALOG}.{SCHEMA}.gold_diagnosis_cost_utilization"
)
GOLD_PATIENT = (
    f"{CATALOG}.{SCHEMA}.gold_patient_utilization"
)
...
monthly_kpis = (
    ...
    .agg(
        ...
        F.countDistinct("patient_id").alias(
            "unique_patient_count"
        ),
        F.countDistinct("provider_id").alias(
        ...
        F.round(
            F.sum("allowed_amount"),
            2,
        ).alias("total_allowed_amount"),
        F.round(
            F.sum("paid_amount"),
            2,
        ...
        F.round(
            F.avg("paid_amount"),
            2,
        ).alias("average_paid_amount"),
        F.sum(
            F.when(
                F.col("claim_type") == "inpatient",
                1,
            ).otherwise(0)
...
# High-cost patients are defined as the top 10% by total paid amount.
HIGH_COST_PERCENTILE = 0.90

high_cost_threshold = patient_kpis_base.agg(
    F.expr(
        f"percentile(total_paid_amount, {HIGH_COST_PERCENTILE})"
    ).alias("threshold")
).first()["threshold"]
...
monthly_kpis.write.format("delta").mode("overwrite").saveAsTable(
    GOLD_MONTHLY
)
provider_kpis.write.format("delta").mode("overwrite").saveAsTable(
    GOLD_PROVIDER
)
diagnosis_kpis.write.format("delta").mode("overwrite").saveAsTable(
    GOLD_DIAGNOSIS
)
patient_kpis.write.format("delta").mode("overwrite").saveAsTable(
    GOLD_PATIENT
)

gold_results = [
    (
        "gold_monthly_cost_utilization",
        monthly_kpis.count(),
    ),
    (
    ...
    (
        "gold_diagnosis_cost_utilization",
        diagnosis_kpis.count(),
    ),
    (
        "gold_patient_utilization",
        patient_kpis.count(),
    ),
]
...
# COMMAND ----------
# Verify final published Gold results.

total_paid_amount = monthly_kpis.agg(
    F.round(
        F.sum("total_paid_amount"),
        2,
    ).alias("value")
).first()["value"]
...
verification_results = [
    ("silver_claim_count", f"{silver_claims.count():,}"),
    (
        "total_paid_amount",
        f"{float(total_paid_amount):,.2f}",
    ),
    (
        "high_cost_definition",
        "P90 — top 10% by total paid amount",
    ),
    (
        "high_cost_threshold",
        f"{float(high_cost_threshold):,.2f}",
    ),
    (
        "high_cost_patient_count",
        f"{high_cost_patient_count:,}",
    ),
]
...
# COMMAND ----------
# M06: High-cost utilization study.
# This is exploratory analysis of synthetic data, not a clinical causal claim.

HIGH_COST_STUDY = (
    f"{CATALOG}.{SCHEMA}.gold_high_cost_utilization_study"
)
...
high_cost_study = (
    ...
    .agg(
        ...
        F.round(
            F.avg("total_paid_amount"),
            2,
        ).alias("average_paid_amount"),
        F.round(
            F.avg("claim_count"),
            2,
        ).alias("average_claim_count"),
        F.round(
            F.avg("inpatient_claim_count"),
            2,
        ).alias("average_inpatient_claim_count"),
        F.round(
            F.avg("risk_score"),
            2,
...
high_cost_study = (
    ...
    .withColumn(
        ...
        F.round(
            F.col("total_paid_amount")
            / F.lit(total_study_paid)
            * 100,
            2,
        ),
    )
    .withColumn(
        "study_published_at_utc",
        F.current_timestamp(),
    )
)
...
display(
    high_cost_study.orderBy(
        F.desc("high_cost_flag")
    )
)

# COMMAND ----------
# Analyze synthetic cost concentration by diagnosis within the high-cost cohort.
...
high_cost_diagnosis_drivers.write.format("delta").mode(
    "overwrite"
).saveAsTable(HIGH_COST_DIAGNOSIS_STUDY)
